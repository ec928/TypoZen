...
- **Never park `await sleep` inside `app.eval` / `page.evaluate`.** One CDP call, one short question. Use `evalPatiently` if the page might be busy. Do not raise `protocolTimeout` (tried at 600s; it does not work).
...
- **`epub-open-app` stall is NOT a product hang.** Isolated, the omnibus is fine: windowing mounts **800 of 45,486** blocks, `goToModelBlock` into the middle takes tens of milliseconds, `getClientRects` over the mounted blocks is ~2 ms. Mark / outline / search jumps in a huge book are fine — do not go looking for a hang there. The suite now **relaunches between Matter and Xeelee**; it is not four books in one instance. Remaining failure is a 25 s `evalPatiently` "main thread busy" on the density stage of the omnibus lap, with leftover in-page sleeps further down `openAndCheck`. Instrument the suite, not the product, and do not raise `protocolTimeout` — that was tried at 600s and only bought ten minutes of waiting for the same failure. See `docs/health-review-2026-08-28-combined.md`.
