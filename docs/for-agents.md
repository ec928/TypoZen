...
- **Disk vs buffer** is host-side (`DocTab.DiskWriteTimeUtc` / `DiskLength` / `DiskFingerprint`). Engine documents only. Check on window Activated, tab switch, folder `FileSystemWatcher` (debounced 400 ms), Save and Autosave. Clean → silent reload. Dirty → prompt. `_e2eMode` skips checks so suites do not get dialogs. After our own atomic save, ignore watcher events for 2 s. Do not poll and do not hash on a timer.
