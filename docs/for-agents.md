...
- **Page totals** are estimates until every range is measured, and the UI says so (`pageTotalIsApproximate` → `43 / ~264`). Three invariants: an unmeasured range's estimate may only **grow** (`setMeasured` takes the max, so a total never revokes a page it has shown); the page you are **on** is always exact; and "the end" is the last **block**, never the last estimated page. Do not assert exact seek against an estimated total — settle the map first (`PageChunks.allMeasured()`), which is what `page-count-truth-app` does.
