...
- **A native tab is a second surface, not a replacement.** PDF / image / media / HTML render in `_nativeWebView`; the editor WebView stays alive behind it holding your document. Two consequences bite. `SendMsg` refuses `cmd:` while `_nativeSurfaceVisible` — every toolbar and menu item routes through it, and without the gate Toggle Sidebar collapsed the sidebar of the document you were *not* looking at, and About opened its modal on the hidden surface. And anything asking "what is the zoom" must read the surface on screen: `ZoomBy` and `WebView_ZoomFactorChanged` both read `_webView` unconditionally, so zoom stuck after one notch on a native tab and the status bar reported a number the content did not match. If something zoom-adjacent misbehaves, that is the shape. Guarded by `native-surface-app.mjs` (50 checks, ~15s) — which also asserts every disabled menu is **given back**, because a menu greyed on a PDF and never restored is worse than the bug. Do not use `TypoZen_Template.html` as an HTML fixture: it runs TypoZen's own JS inside the viewer, including the Ctrl+wheel handler, so it behaves unlike any other page. Use `tests/native-sample.html`.
